...
class TCPDF {
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		// DEPRECATED in PHP 7.2 while (list($key, $val) = each($prop)) {
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					// DEPRECATED in PHP 7.2 while (list($id, $name) = each($attr_array[1])) {
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// DEPRECATED in PHP 7.2 while (list($id, $name) = each($style_array[1])) {
